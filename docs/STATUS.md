...
- 2026-09-21 : PR #45 mergée : runner `TOMATO_AGENT=visible` (Claude Code headless dans une vraie fenêtre Windows Terminal, flux stream-json filmé) ; PR #47 mergée : prises v3 (concepts et cycle avec runner visible, détection et pipeline gratuites, nouvelle caméra + caméra outil), vidéo `data/video/tomato-demo-v3.mp4` (7 min 08 s). Montage v3.1 livré.
- 2026-09-21 : PR #48 mergée : montage v3.1 `data/video/tomato-demo-v3.1.mp4` (7 min 19 s) : sous-titres recalés sur les marqueurs réels (test `realTakes.test.ts`), vignette terminal dès le réveil, deux arrêts sur le terminal (tool_use puis tool_result), `powershell -NoLogo`, attente du `result` 30 s en mode visible. 998 tests. Vidéo livrée, en attente des retours.
