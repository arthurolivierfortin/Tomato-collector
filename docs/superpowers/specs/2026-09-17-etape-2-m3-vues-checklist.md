...
## Code
- [x] [SPEC-1] Bases des caméras (top −Z, front +Y, side −X), lacet autour de Z monde puis tangage autour de l'axe droite, `projectToPixel` pur (800×800, py vers le bas, `pxPerCm = 800 / widthCm`), `imageAxes`, plan de grille par le point visé — `packages/sim/src/cameras/ortho.ts`
- [x] [SPEC-2] Espacement de grille automatique ∈ {1, 2, 5, 10, 20} cm, au plus 16 lignes par côté — `packages/sim/src/cameras/gridSpacing.ts`
- [x] [SPEC-3] Lignes de grille des deux axes monde de l'image, dans le plan visé, projetées (correctes en pivot) ; barre d'échelle 5/10/20 cm — `packages/sim/src/cameras/gridLines.ts`
- [x] [SPEC-4] Fraction de visibilité = pixels comptés ÷ π(r·pxPerCm)², bornée à [0, 1] ; couleurs d'identifiants bijectives 1..255 ; comptage par identifiant — `packages/sim/src/cameras/visibility.ts`
- [x] [SPEC-5] Retournement des lignes du render target et conversion linéaire → sRGB ; noyau Sobel, assombrissement, composition — `packages/sim/src/cameras/pixels.ts`, `edgesCore.ts`
- [x] [SPEC-6] Palette de la spec (rouge/orange/vert, cyan tige, magenta axe lame, blanc-bleu normale, jaune panier, blanc contours, grille 25 %, gris axes), police ≥ 13 px ; commandes d'overlay typées — `packages/sim/src/cameras/palette.ts`, `overlayTypes.ts`
- [x] [SPEC-7] Géométrie du schéma des ciseaux calculée localement (lames 6 cm, pivot = point de coupe − axe·3, pointes à ±ouverture/2 autour de la normale, bouts d'axes à 8 cm) — `packages/sim/src/cameras/scissorsGeometry.ts`
- [x] [SPEC-8] Couche 2 : grille, axes monde marqués, valeurs en cm dans les marges, `X →` / `Z ↑`, barre d'échelle en bas à gauche — `packages/sim/src/cameras/layerGrid.ts`
- [x] [SPEC-9] Couche 3 : cercle numéroté par tomate, couleur d'état, XYZ en cm, badge « occultée » si `visibleIn[cam] < 0,5`, anneau cyan sur la cible — `packages/sim/src/cameras/layerMarkers.ts`
- [x] [SPEC-10] Couche 4 : tige cible cyan (`stem.fromCm` → `stem.toCm`) ; ciseaux (pivot, deux lames, croix du point de coupe, axe lame magenta et normale blanc-bleu, lacet/tangage/roulis/ouverture en texte) ; panier (fond et bord projetés, arêtes, centre en croix) ; verticale de chute pointillée jusqu'au plan du fond du panier avec impact prédit (même X, Y) — `packages/sim/src/cameras/layerTools.ts`
- [x] [SPEC-11] Couche 5 : bandeau haut avec nom de la vue, axes visibles, position et angles de la caméra, px/cm, espacement et plan de grille, temps sim, mention PIVOTÉE — `packages/sim/src/cameras/layerHeader.ts` ; assemblage des couches — `annotations.ts`
- [x] [SPEC-12] JSON des vues (`ViewsPayload` de `@tomato/shared`) construit depuis le store : sim time, phase, cible, tomates (état, ripeness, position, stem, visibleIn), ciseaux, panier, caméras, limites — `packages/sim/src/cameras/payload.ts`
- [x] [SPEC-13] Action `move_camera` : déplacement relatif clampé aux rails (`fail('out_of_rail', …, { axis, requestedCm, minCm, maxCm })`), lacet/tangage bornés à ±`cameraPivotDeg`, zoom > 0 divisant `widthCm` dans [20, 200] cm, `pxPerCm` recalculé, jamais de `throw` — `packages/sim/src/cameras/cameraState.ts`
- [x] [SPEC-14] Trois `OrthographicCamera` + `WebGLRenderTarget` 800×800 + `CameraHelper` dans la scène spectateur ; `poseCamera` par la base monde convertie avec `worldToThree` ; lecture du render target en `ImageData` — `packages/sim/src/cameras/agentCameras.ts`
- [x] [SPEC-15] Passe d'identifiants : matériau plat par tomate (couleur en linéaire), occulteurs noirs conservant `map`/`alphaTest`/`side`, non-Mesh masqués, fond noir, scène restaurée — `packages/sim/src/cameras/idPass.ts`
- [x] [SPEC-16] Interface `EdgeFilter = (img: ImageData) => ImageData` avec Sobel par défaut, `darken`, `compose` — `packages/sim/src/cameras/edges.ts`
- [x] [SPEC-17] Dessin des commandes sur canvas 2D avec halo sombre sous chaque texte — `packages/sim/src/cameras/drawOverlay.ts`
...
- [x] [TEST-2] `packages/sim/src/cameras/gridSpacing.test.ts` — 8 px/cm → 10 cm, 16 → 5, 4 → 20, 40 → 2, 80 → 1 ; jamais plus de 16 lignes
- [x] [TEST-3] `packages/sim/src/cameras/gridLines.test.ts` — lignes X verticales à px = 400 + X·8, lignes Z horizontales, alignées monde en tangage ; barre d'échelle 20/10/5 cm
- [x] [TEST-4] `packages/sim/src/cameras/visibility.test.ts` — fraction bornée ; bijection 1..255 ; comptage par identifiant
- [x] [TEST-5] `packages/sim/src/cameras/pixels.test.ts`, `edgesCore.test.ts` — retournement des lignes, LUT sRGB, Sobel sur une marche, seuil, assombrissement, composition
- [x] [TEST-7] `packages/sim/src/cameras/annotations.test.ts` (`scissorsPoints`) — pivot à 3 cm derrière le point de coupe, pointes à 6 cm écartées par l'ouverture
- [x] [TEST-8] `packages/sim/src/cameras/annotations.test.ts` — grille, étiquettes d'axes et barre d'échelle présentes
- [x] [TEST-9] `packages/sim/src/cameras/annotations.test.ts` — deux marqueurs (couleurs d'état, XYZ) ; badge « occultée » dans la vue où `visibleIn < 0,5` et pas ailleurs
- [x] [TEST-10] `packages/sim/src/cameras/annotations.test.ts` — tige cible cyan de `fromCm` à `toCm` ; schéma des ciseaux (deux lames, pivot, croix, deux axes de couleurs différentes, texte des angles) ; panier (deux rectangles, centre) ; verticale de chute jusqu'au z du fond du panier ; rien de spécifique sans cible
- [x] [TEST-11] `packages/sim/src/cameras/annotations.test.ts` — bandeau (VUE FRONT, axes, pose caméra, px/cm, temps sim, PIVOTÉE) ; textes ≥ 13 px avec halo
- [x] [TEST-12] `packages/sim/src/cameras/payload.test.ts` — payload copie l'état ; `TomatoView` sans rayon
...
- [x] [GATE-5] npm run shot → `data/shots/scene.png`, `view-top.png`, `view-front.png`, `view-side.png` produits
