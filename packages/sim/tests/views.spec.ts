...
test('renderViews returns three annotated 800×800 views with their JSON and captures them', async ({ page }) => {
  ...
  // Issue #21 : les mouvements sont animés. La mise en place n'a pas à être jouée en temps réel :
  // elle utilise `applyNow`, la variante immédiate du runtime (pose finale d'un coup, mêmes contrôles).
  const setup = await page.evaluate(() => {
    ...
    const basket = rt.applyNow({ type: 'move_basket', x: clamp(tx, rail.x[0], rail.x[1]), y: clamp(ty, rail.y[0], rail.y[1]), mode: 'absolute' });
    ...
    const scissors = rt.applyNow({ type: 'move_scissors', x: cut[0], y: cut[1], z: cut[2], mode: 'absolute' });
    const open = rt.applyNow({ type: 'open_scissors' });
    return {
      targetId: target.id,
